...
export const BottomNavigation: React.FC = () => {
  ...
  return (
    // Outer container: Fixed at bottom, with background covering the entire safe area
    // Using bg-wafu-paper/95 ensures no "underlying" content shows through
    <div className="fixed bottom-0 left-0 w-full md:w-auto md:left-1/2 md:-translate-x-1/2 md:bottom-8 z-50 transition-all duration-300 bg-wafu-paper/95 backdrop-blur-xl border-t border-wafu-indigo/5 md:bg-transparent md:backdrop-blur-none md:border-none">
       {/* 
          Inner container: Handles content layout and padding
          Use inline style for paddingBottom to robustly handle env(safe-area-inset-bottom)
          without Tailwind's arbitrary value parser potentially breaking on complex functions.
          Logic: 4px base padding + Safe Area. This moves icons down to "the very bottom" while respecting iPhone Home Bar.
       */}
       <div 
         className="md:bg-wafu-paper/95 md:backdrop-blur-xl md:border md:border-wafu-indigo/10 md:rounded-full md:shadow-2xl shadow-[0_-5px_20px_rgba(24,54,84,0.02)] pt-2 md:pb-1 md:px-8 w-full md:w-auto"
