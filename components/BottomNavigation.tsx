...
export const BottomNavigation: React.FC<BottomNavigationProps> = ({ activeTab, setActiveTab, shoppingCount }) => {
  return (
    // 修正：移除多餘 padding，僅保留 safe-area，並確保背景色延伸到底部，消除視覺上的白色空隙
    <div className="fixed bottom-0 left-0 w-full bg-wafu-paper/95 backdrop-blur-xl border-t border-stone-200/60 z-40 shadow-[0_-5px_25px_rgba(24,54,84,0.05)] pb-[env(safe-area-inset-bottom)] transition-all">
      
      <div className="max-w-lg mx-auto w-full grid grid-cols-6 px-1 py-2">
        <button onClick={() => setActiveTab('itinerary')} className={`relative z-10 group flex flex-col items-center justify-center gap-1 transition-all duration-300 active-bounce ${activeTab === 'itinerary' ? 'text-wafu-indigo' : 'text-stone-400 hover:text-wafu-indigo/60'}`}>
        ...
        <button onClick={() => setActiveTab('sightseeing')} className={`relative z-10 group flex flex-col items-center justify-center gap-1 transition-all duration-300 active-bounce ${activeTab === 'sightseeing' ? 'text-wafu-indigo' : 'text-stone-400 hover:text-wafu-indigo/60'}`}>
        ...
        <button onClick={() => setActiveTab('food')} className={`relative z-10 group flex flex-col items-center justify-center gap-1 transition-all duration-300 active-bounce ${activeTab === 'food' ? 'text-wafu-indigo' : 'text-stone-400 hover:text-wafu-indigo/60'}`}>
        ...
        <button onClick={() => setActiveTab('money')} className={`relative z-10 group flex flex-col items-center justify-center gap-1 transition-all duration-300 active-bounce ${activeTab === 'money' ? 'text-wafu-indigo' : 'text-stone-400 hover:text-wafu-indigo/60'}`}>
        ...
        <button onClick={() => setActiveTab('shop')} className={`relative z-10 group flex flex-col items-center justify-center gap-1 transition-all duration-300 active-bounce ${activeTab === 'shop' ? 'text-wafu-indigo' : 'text-stone-400 hover:text-wafu-indigo/60'}`}>
        ...
        <button onClick={() => setActiveTab('flight')} className={`relative z-10 group flex flex-col items-center justify-center gap-1 transition-all duration-300 active-bounce ${activeTab === 'flight' ? 'text-wafu-indigo' : 'text-stone-400 hover:text-wafu-indigo/60'}`}>
