...
export const useFirestoreData = () => {
  ...
  useEffect(() => {
    // 檢查 Firebase DB 是否已初始化
    // 如果因為網路問題或腳本載入失敗導致 db 未定義，這裡會攔截錯誤，防止 App 崩潰
    if (!db) {
        console.error("Firebase DB 未初始化，無法載入資料。請檢查 API Key 或網路連線。");
        setDbError(true);
        return;
    }
    ...
    try {
        ...
        const unsubItinerary = db.collection('itinerary').onSnapshot(async (snapshot: any) => {
          // 初始化邏輯 (Data Seeding)
          ...
          if (snapshot.empty && !seedAttempted.current) {
             ...
             const items = snapshot.docs.map((doc: any) => ({ id: doc.id, ...doc.data() } as ItineraryItem));
        ...
        // 2. 監聽 'expenses' (支出) 集合
        const unsubExpenses = db.collection('expenses').onSnapshot((snapshot: any) => {
          const items = snapshot.docs.map((doc: any) => ({ id: doc.id, ...doc.data() } as Expense));
          setExpenses(items);
        }, handleSnapshotError);

        // 3. 監聽 'shopping' (購物清單) 集合
        const unsubShopping = db.collection('shopping').onSnapshot((snapshot: any) => {
          const items = snapshot.docs.map((doc: any) => ({ id: doc.id, ...doc.data() } as ShoppingItem));
          setShoppingItems(items);
        }, handleSnapshotError);

        // 4. 監聽 'restaurants' (餐廳) 集合
        const unsubRestaurants = db.collection('restaurants').onSnapshot((snapshot: any) => {
          const items = snapshot.docs.map((doc: any) => ({ id: doc.id, ...doc.data() } as Restaurant));
          setRestaurants(items);
        }, handleSnapshotError);

        // 5. 監聽 'sightseeing' (景點) 集合
        const unsubSightseeing = db.collection('sightseeing').onSnapshot((snapshot: any) => {
          const items = snapshot.docs.map((doc: any) => ({ id: doc.id, ...doc.data() } as SightseeingSpot));
        ...
        console.error("建立 Snapshot 監聽器時發生錯誤:", e);
        setDbError(true);
    }
  }, []);
